...
namespace GarbageCollector {	
	...
	private:
		std::map< Object*, std::pair< SpaceType, bool > >::const_iterator edgelist_iter_begin, edgelist_iter_end;
		std::map< Object*, std::pair< SpaceType, bool > >::const_iterator vertexlist_iter_begin, vertexlist_iter_end;
	public:
	...
	void CollectAllGarbage() {
		...
		for( auto s = ObjectMap.begin(); s != ObjectMap.end(); ++s ) {
			if( s->second.first == Stack ) {
				g.dfs< Original<std::pair< Object*, std::pair< SpaceType, bool > > > >( *s );
			}
		}
...
//				std::cout << "Marking: " << iter->first << std::endl;
		...
		for( auto s = ObjectMap.begin(); s != ObjectMap.end(); ) {
			if( !s->second.second ) {
